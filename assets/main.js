...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const searchResultsList = document.getElementById('searchResultsList'); // Container for search results table
    const noSearchResultsMessage = document.getElementById('noSearchResults'); // "No emails found" message
    ...
    // Image Selector Modal specific inputs
    ...
    const apiCall = async (endpoint, data) => {
        try {
            // Normalize base_url to ensure it ends with a single slash
            const baseUrl = AppConfig.base_url.endsWith('/') ? AppConfig.base_url : `${AppConfig.base_url}/`;
            // Normalize endpoint to ensure it doesn't start with a slash, then prepend 'api/'
            const apiPath = `api/${endpoint.startsWith('/') ? endpoint.substring(1) : endpoint}`;

            const url = `${baseUrl}${apiPath}`;
            console.log('Constructed API URL:', url); // IMPORTANT: Log the final URL for debugging
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    errorMessage += `, response: ${errorText.substring(0, 100)}...`; // Truncate for display
                }
    ...
    const showConfirmationModal = (title, message, onConfirm, onCancel = null, isAlert = false) => {
        if (!confirmationModal || !confirmationModalTitle || !confirmationModalMessage || !confirmYesBtn || !confirmNoBtn || !confirmOkBtn) {
            console.error('Confirmation modal elements not found, falling back to alert.');
            alert(`${title}: ${message}`); // Fallback to alert
            if (onConfirm) onConfirm(); // Still call the callback if it's an alert
            return;
        }
        ...
        // Remove existing listeners to prevent duplicates and add new ones using `once: true`
        // Ensure these are cleared or used with `once: true`
        confirmYesBtn.removeEventListener('click', confirmationHandler);
        confirmNoBtn.removeEventListener('click', confirmationHandler);
        confirmOkBtn.removeEventListener('click', confirmationHandler);

        const confirmationHandler = (event) => {
            hideModal(confirmationModal);
            if (event.target === confirmYesBtn || event.target === confirmOkBtn) {
                if (confirmationCallback) confirmationCallback();
            } else if (event.target === confirmNoBtn) {
                if (onCancel) onCancel();
            }
        };

        if (isAlert) {
            confirmOkBtn.addEventListener('click', confirmationHandler, { once: true });
        } else {
            confirmYesBtn.addEventListener('click', confirmationHandler, { once: true });
            confirmNoBtn.addEventListener('click', confirmationHandler, { once: true });
        }
    ...
    const handleLayoutFormSubmit = async () => {
        ...
        if (referenceCode) {
            ...
            try {
                ...
                const response = await apiCall('fetch_email.php', { reference_code: referenceCode });
                if (!response.success || !response.data) {
                    ...
                    currentReferenceCode = emailData.reference_code; // Ensure currentReferenceCode is set from fetched data
        ...
        renderArticleBlocks(); // Re-render blocks based on updated articleBlocks array
    ...
    const resetEditorState = (initialData = null) => {
        ...
        currentPersonalizationType = initialData?.personalization_type || 'none';
    ...
    const populateForms = (emailData = null) => {
        ...
        if(emailData?.created_by) {
            createdByDisplaySpan.textContent = emailData.created_by;
        ...
        subjectLineInput.value = emailData?.subject || '';
        preheaderTextInput.value = emailData?.preheader || '';
        backgroundImageInput.value = emailData?.main_image || '';
        regionSelect.value = emailData?.region || 'UKI'; // Default to UKI
        emailFromInput.value = emailData?.from_email || '';
        senderNameInput.value = emailData?.sender_name || '';
        senderEmailInput.value = emailData?.sender_email || '';
        audienceSelect.value = emailData?.audience || 'channel'; // Added
        sendTimeInput.value = emailData?.send_time || ''; // Added, assuming 'send_time' from backend
        logoUrlInput.value = emailData?.logo_url || AppConfig.default_logo_url; // Added
        senderMobileInput.value = emailData?.sender_mobile || ''; // Added
                        ...
                        handlers: {
                            'image': function() { // Custom handler for Quill's built-in image button
                                ...
                                if (range) {
                                    ...
                                    imageSelectorModal.dataset.targetQuillEditorId = editorId; // Store which Quill editor to update
                                    imageSelectorModal.dataset.targetInputId = ''; // Clear direct input target
                                    // Make sure selectedImageUrlInput is cleared and image gallery section is visible/up-to-date
                                    if (selectedImageUrlInput) selectedImageUrlInput.value = '';
    ...
    const attachArticleBlockListeners = (blockElement, index) => {
        ...
        blockElement.querySelectorAll('button.select-image-button').forEach(button => {
            button.addEventListener('click', (event) => {
                ...
                imageSelectorModal.dataset.targetQuillEditorId = ''; // Clear Quill editor target
                // Make sure selectedImageUrlInput is cleared before showing modal
                if (selectedImageUrlInput) selectedImageUrlInput.value = document.getElementById(targetInputId)?.value || ''; // Pre-fill with current value
                showModal(imageSelectorModal); // Open the image selector modal
            });
        });
    };
    ...
    const getCurrentFormData = () => {
        const data = {
            ...
            subject: subjectLineInput.value,
            preheader: preheaderTextInput.value,
            main_image: backgroundImageInput.value,
            region: regionSelect.value,
            from_email: emailFromInput.value,
            sender_name: senderNameInput.value,
            sender_email: senderEmailInput.value,
            personalization_type: currentPersonalizationType,
            created_by: AppConfig.logged_in_user_name, // User who is logged in and creating/saving
            audience: audienceSelect.value, // Added
            send_time: sendTimeInput.value, // Added, assuming 'send_time' from backend
            logo_url: logoUrlInput.value, // Added
            sender_mobile: senderMobileInput.value, // Added
            article_blocks: []
        };
    ...
    const performEmailSearch = async () => {
        ...
        try {
            const results = await apiCall('search_emails.php', searchParams);
    ...
    const displaySearchResults = (results) => {
        ...
        let table = document.getElementById('searchResultsTable'); // Check if table already exists in the DOM
        if (!table) { // If not, create it
            table = document.createElement('table');
            table.id = 'searchResultsTable';
            table.className = 'search-results-table'; // For styling
            table.innerHTML = `
                <thead>
                    <tr>
                        <th>Ref Code</th>
        ...
        results.forEach(email => {
            ...
            row.insertCell().textContent = email.reference_code || 'N/A';
            row.insertCell().textContent = email.subject || 'N/A';
            row.insertCell().textContent = email.created_by || 'N/A';
            row.insertCell().textContent = email.sender_email || 'N/A';
            row.insertCell().textContent = email.region || 'N/A';
            row.insertCell().textContent = email.last_modified || 'N/A';
            ...
            loadButton.addEventListener('click', () => {
                showModalMessage('Loading Email', `Loading email ${email.reference_code}...`);
                hideModal(searchEmailModal); // Close the search modal immediately
                loadEmail(email.reference_code); // Call the load function
            });
            ...
            deleteButton.addEventListener('click', () => confirmDeleteEmail(email.reference_code));
            ...
            async () => {
                try {
                    ...
                    if (response.success && response.data) {
                        ...
                        resetEditorState(emailData); // This will clear existing and re-populate articleBlocks array
    ...
    const resetEditor = () => {
        ...
        // Reset personalization radio to 'none' and update UI
        currentPersonalizationType = 'none';
        const noneRadio = document.querySelector('input[name="personalizationType"][value="none"]');
    ...
    const handleSaveAsEmail = async () => {
        ...
        try {
            const checkResponse = await apiCall('check_reference_code.php', { reference_code: newRefCode });
        ...
        const formData = getCurrentFormData();
        formData.reference_code = newRefCode; // Override with new ref code for saving
        formData.subject = newTitle; // Override with new title for saving
        // The `created_by` field is already correctly set in `getCurrentFormData` for the current user
    ...
    const handleBlockSearch = async () => {
        ...
        try {
            const response = await apiCall('search_blocks.php', { search_term: searchTerm });
            if (response.success) {
    ...
    const loadBlock = async (blockId) => {
        ...
        try {
            const response = await apiCall('load_block.php', { block_id: blockId });
            if (response.success && response.data) {
    ...
    const confirmDeleteBlock = (blockId, blockName) => {
        showConfirmationModal(
            ...
            async () => {
                try {
                    const response = await apiCall('delete_block.php', { block_id: blockId });
                    if (response.success) {
    ...
    const saveCurrentQuillContentAsBlock = async (blockName, blockType) => {
        ...
        try {
            const response = await apiCall('save_block.php', {
    ...
    const handleAddPersonalizationTag = async () => {
        ...
        try {
            const response = await apiCall('add_personalization_tag.php', { tag: newTag });
            if (response.success) {
    ...
    const fetchAndDisplayPersonalizationTags = async () => {
        try {
            const response = await apiCall('get_personalization_tags.php', {});
            if (response.success && Array.isArray(response.data)) {
                availableTagsList.innerHTML = ''; // Clear modal list
                personalizationTagListInDropdown.innerHTML = ''; // Clear dropdown list

                if (response.data.length === 0) {
                    availableTagsList.innerHTML = '<li>No tags available.</li>';
                    personalizationTagListInDropdown.innerHTML = '<li>No tags available.</li>';
                    return;
                }
                ...
                availableTagsList.innerHTML = '<li>Error loading tags.</li>';
                personalizationTagListInDropdown.innerHTML = '<li>Error loading tags.</li>';
            }
        } catch (error) {
    ...
    const insertPersonalizationTag = (tag) => {
        if (currentQuillEditor) {
            ...
            if (range) {
                currentQuillEditor.insertText(range.index, tag);
                currentQuillEditor.setSelection(range.index + tag.length);
            } else {
                // If no selection, insert at the beginning or end of content
                currentQuillEditor.insertText(currentQuillEditor.getLength(), tag);
                currentQuillEditor.setSelection(currentQuillEditor.getLength());
            }
    ...
    const handleImageInsertion = () => {
        const imageUrl = selectedImageUrlInput.value.trim();
        const targetInputId = imageSelectorModal.dataset.targetInputId;
        const targetQuillEditorId = imageSelectorModal.dataset.targetQuillEditorId;

        if (!imageUrl) {
            showModalMessage('Selection Required', 'Please enter or select an image URL first.');
            return;
        }
        ...
        if (targetQuillEditorId && quillEditors[targetQuillEditorId]) {
            ...
            targetQuillEditor.insertEmbed(range.index, 'image', imageUrl);
            ...
            if (targetInput) {
                ...
                showModalMessage('Image Selected', 'Image URL inserted!');
        ...
        hideModal(imageSelectorModal);
        // generateHtmlAndPreview(); // This is handled by the input event dispatch or Quill's text-change
    };
    ...
    const setupEventListeners = () => {
        ...
        if (openSearchModalBtn) {
            openSearchModalBtn.addEventListener('click', () => {
                ...
                searchSubjectLineInput.value = '';
                searchReferenceCodeInput.value = '';
                searchAuthorInput.value = '';
                searchSenderEmailInput.value = '';
        ...
        if (mainBackgroundImageSelectBtn) {
            mainBackgroundImageSelectBtn.addEventListener('click', (event) => {
                imageSelectorModal.dataset.targetInputId = 'backgroundImage'; // Set target input ID
                imageSelectorModal.dataset.targetQuillEditorId = ''; // Clear Quill target
                if (selectedImageUrlInput) selectedImageUrlInput.value = backgroundImageInput.value; // Pre-fill
                showModal(imageSelectorModal);
            });
        }
            ...
            generateAndSaveEmailBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (!formData.reference_code) {
                        formData.reference_code = generateReferenceCode();
                    }
                    const response = await apiCall('save_email.php', formData);
                    if (response.success) {
                        ...
                        currentReferenceCode = formData.reference_code;
                        referenceCodeInput.value = currentReferenceCode;
                        displayReferenceCodeSpan.textContent = currentReferenceCode;
                        currentReferenceCodeDisplayDiv.style.display = 'block';
                    } else {
        ...
        if (downloadHtmlBtn) {
            downloadHtmlHtmlBtn.addEventListener('click', () => {
        ...
        if (saveAsEmailBtn) {
            saveAsEmailBtn.addEventListener('click', () => {
                newReferenceCodeInput.value = ''; // Clear for new input
                newEmailTitleInput.value = subjectLineInput.value || ''; // Pre-fill with current subject
        ...
        if (imageSelectorModal) {
            imageSelectorModal.querySelector('#insertImageBtn').addEventListener('click', handleImageInsertion);
        }
        ...
        document.body.addEventListener('click', (event) => {
            if (event.target.classList.contains('close-button')) {
                ...
                const modalToClose = event.target.closest('.app-modal') ||
                                     ...
                                     event.target.closest('#confirmationModal');
                if (modalToClose) {
    ...
    const initializeApp = () => {
        setupEventListeners(); // Set up all event listeners once the DOM is ready

        // Check for an initial reference code in the URL to load an email
        const urlParams = new URLSearchParams(window.location.search);
        const initialReferenceCode = urlParams.get('code');

        if (initialReferenceCode) {
            referenceCodeInput.value = initialReferenceCode;
            handleLayoutFormSubmit(); // Automatically load the email if code is present
        } else {
            resetEditor(); // Start with a new, empty email if no code is in URL
        }

        console.log('EmailPOS App Initialized with AppConfig:', AppConfig);
    };

    // Helper function to update personalization UI based on selected type
    const updatePersonalizationUI = () => {
        // This function will be called whenever currentPersonalizationType changes
        // It rebuilds the personalization dropdown based on the new type
        if (personalizationButton && personalizationTagListInDropdown) {
            if (currentPersonalizationType === 'salesforce_list') {
                personalizationButton.style.display = 'inline-block';
                personalizationTagListInDropdown.innerHTML = `
                    <span class="personalization-option" data-value="{{{Recipient.FirstName}}}">Firstname</span>
                    <span class="personalization-option" data-value="{{{Recipient.LastName}}}">Lastname</span>
                    <span class="personalization-option" data-value="{{{Recipient.Title}}}">Job Title</span>
                    <span class="personalization-option" data-value="{{{Recipient.Account_Name_for_Email_Template__c}}}">Companyname</span>
                    <span class="personalization-option" data-value="{{{Recipient.Email}}}">Email</span>
                `;
            } else if (currentPersonalizationType === 'salesforce_marketing_cloud') {
                personalizationButton.style.display = 'inline-block';
                personalizationTagListInDropdown.innerHTML = `
                    <span class="personalization-option" data-value="%%FirstName%%">Firstname</span>
                    <span class="personalization-option" data-value="%%LastName%%">Lastname</span>
                    <span class="personalization-option" data-value="%%JobTitle%%">Job Title</span>
                    <span class="personalization-option" data-value="%%CompanyName%%">Companyname</span>
                    <span class="personalization-option" data-value="%%Email%%">Email</span>
                `;
            } else { // 'none'
            ...
            // Attach event listeners to newly created personalization options
            personalizationTagListInDropdown.querySelectorAll('.personalization-option').forEach(span => {
