...
// Totals and charts: aggregate in EUR (correct mixed INR/EUR rows), then convert to display currency when not EUR.
...
if ($selected_region !== 'all') {
    $spend_by_account_type = getSpendByAccountTypeInEUR($pdo, $selected_region, $whereClause, $params);
    $spend_by_amplify_level = getSpendByAmplifyLevelInEUR($pdo, $selected_region, $whereClause, $params);
    $top_spending_vendors = getTopSpendingVendorsInEUR($pdo, $selected_region, 10, $whereClause, $params);
    $vendor_activity_timeline = getVendorActivityTimelineInEUR($pdo, $selected_region, $whereClause, $params);
} else {
    $spend_by_account_type = getSpendByAccountTypeInEUR($pdo, 'all', $whereClause, $params);
    $spend_by_amplify_level = getSpendByAmplifyLevelInEUR($pdo, 'all', $whereClause, $params);
    $top_spending_vendors = getTopSpendingVendorsInEUR($pdo, 'all', 10, $whereClause, $params);
    $vendor_activity_timeline = getVendorActivityTimelineInEUR($pdo, 'all', $whereClause, $params);
}
$spend_by_account_type = convertReportRowsMoneyFromEUR($spend_by_account_type, ['total_spent'], $currency, $pdo);
$spend_by_amplify_level = convertReportRowsMoneyFromEUR($spend_by_amplify_level, ['total_spent'], $currency, $pdo);
$top_spending_vendors = convertReportRowsMoneyFromEUR($top_spending_vendors, ['total_spent', 'avg_spend_per_item'], $currency, $pdo);
$vendor_activity_timeline = convertReportRowsMoneyFromEUR($vendor_activity_timeline, ['monthly_spend'], $currency, $pdo);
...
$current_year_data = convertReportSeriesFromEUR(
    getMonthlyTrendDataInEUR($pdo, $selected_year, $whereClause, $params),
    $currency,
    $pdo
);
$previous_year_data = convertReportSeriesFromEUR(
    getMonthlyTrendDataInEUR($pdo, $selected_year - 1, $whereClause, $params),
    $currency,
    $pdo
);
$quarterly_data = convertReportSeriesFromEUR(
    getQuarterlyDataInEUR($pdo, $selected_year, $whereClause, $params),
    $currency,
    $pdo
);
...
$matched_data = convertReportRowsMoneyFromEUR(
    getMatchedUnmatchedDataInEUR($pdo, $selected_year, $whereClause, $params),
    ['total'],
    $currency,
    $pdo
);
