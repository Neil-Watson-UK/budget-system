...
function getRegionalSpent($pdo, $region, $year = null, $status_filter = null) {
    $sql = "SELECT SUM(amount_requested) as total FROM budget_items WHERE region = ?";
    $params = [$region];
    
    // Add year filter if specified - FIXED: Check for 'all'
    if ($year && $year !== 'all') {
        $sql .= " AND (YEAR(start_date) = ? OR YEAR(end_date) = ? OR (YEAR(start_date) IS NULL AND YEAR(entry_creation_date) = ?))";
        $params[] = $year;
        $params[] = $year;
        $params[] = $year;
    }
...
function getRegionalSpentWithFilters($pdo, $region_or_where, $year_or_params = null, $status_filter = null, $additional_filters = []) {
    ...
    if (is_string($region_or_where) && is_array($year_or_params)) {
        $sql = "SELECT SUM(amount_requested) as total FROM budget_items WHERE " . $region_or_where;
        $stmt = $pdo->prepare($sql);
...
function getRegionalItems($pdo, $region, $year = null, $status_filter = null, $additional_filters = []) {
    $sql = "SELECT * FROM budget_items WHERE region = ?";
    $params = [$region];
...
function getAvailableYearsForRegion($pdo, $region) {
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$region, $region, $region]);
...
function getGlobalSummary($pdo, $year = null) {
    ...
    $query = "
        SELECT 
            ...
            COUNT(*) as total_items
        FROM budget_items 
        WHERE (YEAR(start_date) = :year OR (start_date IS NULL AND YEAR(entry_creation_date) = :year))
        GROUP BY region
        ORDER BY region
    ";
    ...
    foreach ($REGIONAL_SETTINGS as $region => $settings) {
        $budget_limit = $settings['budget'] ?? 0;
...
function getAllYearsSummary($pdo) {
    ...
    // Add budget data from regional settings
    global $REGIONAL_SETTINGS;
    foreach ($consolidated as &$year_data) {
        $total_budget = 0;
        foreach ($REGIONAL_SETTINGS as $region => $settings) {
            $total_budget += $settings['budget'] ?? 0;
        }
        $year_data['budget'] = $total_budget;
    }
...
function getSpendByAccountType($pdo, $region, $whereClause, $params) {
    $sql = "SELECT COALESCE(v.account_type, 'Other') as account_type, SUM(bi.amount_requested) as total_spent
        FROM budget_items bi
        LEFT JOIN vendors v ON TRIM(LOWER(v.vendor_name)) = TRIM(LOWER(bi.vendor))
        WHERE $whereClause
        GROUP BY COALESCE(v.account_type, 'Other') ORDER BY total_spend DESC";
    try {
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
...
function getSpendByAmplifyLevel($pdo, $region, $whereClause, $params) {
    try {
        $sql = "SELECT COALESCE(v.AMPLIFY_Level__c, 'Other') as amplify_level, SUM(bi.amount_requested) as total_spent
            FROM budget_items bi
            LEFT JOIN vendors v ON TRIM(LOWER(v.vendor_name)) = TRIM(LOWER(bi.vendor))
            WHERE $whereClause
            GROUP BY COALESCE(v.AMPLIFY_Level__c, 'Other') ORDER BY total_spend DESC";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
...
function getVendorActivityTimeline($pdo, $region, $whereClause, $params) {
    $sql = "SELECT DATE_FORMAT(bi.start_date, '%Y-%m') as month, COALESCE(v.account_type, 'Other') as account_type,
        ...
        FROM budget_items bi
        LEFT JOIN vendors v ON TRIM(LOWER(v.vendor_name)) = TRIM(LOWER(bi.vendor))
        WHERE $whereClause AND bi.start_date IS NOT NULL
...
function getMonthlyTrendData($pdo, $year, $whereClause, $params) {
    $yearCond = " AND (YEAR(start_date) = ? OR YEAR(end_date) = ? OR (YEAR(start_date) IS NULL AND YEAR(entry_creation_date) = ?))";
    $p = array_merge($params, [$year, $year, $year]);
...
function getStatusDistributionData($pdo, $year, $whereClause, $params) {
    $yearCond = ($year && $year !== 'all') ? " AND (YEAR(start_date) = ? OR YEAR(end_date) = ? OR (YEAR(start_date) IS NULL AND YEAR(entry_creation_date) = ?))" : "";
    $p = ($year && $year !== 'all') ? array_merge($params, [$year, $year, $year]) : $params;
...
function getMatchedUnmatchedData($pdo, $year, $whereClause, $params) {
    $yearCond = ($year && $year !== 'all') ? " AND (YEAR(bi.start_date) = ? OR YEAR(bi.end_date) = ? OR (YEAR(bi.start_date) IS NULL AND YEAR(bi.entry_creation_date) = ?))" : "";
    $yearParams = ($year && $year !== 'all') ? [$year, $year, $year] : [];
