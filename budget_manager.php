...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_budget'])) {
        ...
        $amount_in = $_POST['amount_in'] ?? 'local'; // 'local' or 'eur'
        ...
        if (empty($region)) {
            ...
            // If user entered in EUR, convert to local currency for storage
            if ($amount_in === 'eur' && $amount > 0) {
                $amount = convertToLocal($amount, $local_currency, $pdo);
...
// Calculate totals (convert to EUR for consistent display)
$yearly_totals = [];
foreach ($budgets_by_year as $year => $year_budgets) {
    $total = 0;
    foreach ($year_budgets as $budget) {
        $total += ($budget['currency'] === 'EUR') 
            ? $budget['budget_amount'] 
            : convertToEUR($budget['budget_amount'], $budget['currency'], $pdo);
    }
...
<body>
    ...
    <div class="container mt-5">
                                ...
                                <form method="post" class="row g-2">
                                    <div class="col-md-5">
                                        <select name="copy_from_year" class="form-select form-select-sm" required>
                                            ...
                                            <?php 
                                            $copy_from_years = array_unique(array_merge(array_keys($budgets_by_year), $available_years));
                                            rsort($copy_from_years);
                                            foreach ($copy_from_years as $y): ?>
        ...
        <?php 
        // Show years with budgets or current year (no future years)
        $display_years = array_keys($budgets_by_year);
        if (!in_array($current_year, $display_years)) {
            $display_years[] = $current_year;
        }
        $display_years = array_filter($display_years, fn($y) => (int)$y <= $current_year);
        rsort($display_years);
        ?>
        ...
        <th>Action</th>
                            ...
                            <tbody>
                                ...
                                foreach ($REGIONAL_SETTINGS as $region => $settings): 
                                    ...
                                    $row_total += ($currency === 'EUR') ? $amount : convertToEUR($amount, $currency, $pdo);
    ...
    <td>
        <div class="input-group input-group-sm">
            <span class="input-group-text curr-symbol"><?= $CURRENCY_SYMBOLS[$currency] ?? '€' ?></span>
            <input type="number" 
                   name="amount[<?= $region ?>]" 
    ...
    <td>
        <select name="amount_in[<?= $region ?>]" class="form-select form-select-sm enter-in-select" 
                data-region="<?= $region ?>" data-local="<?= htmlspecialchars($currency) ?>" 
                data-symbol="<?= htmlspecialchars($CURRENCY_SYMBOLS[$currency] ?? '€') ?>" style="width: 100px;">
            <option value="local" selected><?= $currency ?> (<?= $CURRENCY_SYMBOLS[$currency] ?? '€' ?>)</option>
...
<script>
    ...
    function saveSingleBudget(button) {
        ...
        const amountInSelect = row.querySelector('select[name^="amount_in["]');
        
        const amount = amountInput.value;
        const amountIn = amountInSelect ? amountInSelect.value : 'local';
        ...
        currencyInput.value = amountInSelect ? amountInSelect.getAttribute('data-local') : 'EUR';
        form.appendChild(currencyInput);
        
        const amountInInput = document.createElement('input');
        amountInInput.type = 'hidden';
        amountInInput.name = 'amount_in';
        amountInInput.value = amountIn;
        form.appendChild(amountInInput);
    ...
    // Auto-update currency symbol when "Enter in" changes
    document.addEventListener('change', function(e) {
        if (e.target.name && e.target.name.startsWith('amount_in[')) {
            const row = e.target.closest('tr');
            const select = e.target;
            const symbol = select.selectedOptions[0].text.match(/\(([^)]+)\)/)?.[1] || '€';
            const currencySpan = row.querySelector('.curr-symbol');
            if (currencySpan) currencySpan.textContent = symbol;
        }
    });
