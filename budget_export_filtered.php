...
if (!empty($year_filter)) {
    $where_clauses[] = budgetYearMatchSql();
    $params = array_merge($params, budgetYearMatchParams($year_filter));
}
...
$sql = "SELECT 
            ...
            start_date, end_date, invoiced_date, budget_accrual_approved,
...
fputcsv($output, [
    ...
    'Start Date', 'End Date', 'Invoiced Date', 'Budget Accrual Approved',
...
foreach ($items as $item) {
    fputcsv($output, [
        ...
        isset($item['budget_accrual_approved']) ? (int)$item['budget_accrual_approved'] : 1,
